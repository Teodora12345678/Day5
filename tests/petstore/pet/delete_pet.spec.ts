...
test("Create and delete a pet", async({request}) => {
    const response = await request.post(`${process.env.API_URL}pet`, {
        data : {
            "id": Date.now(),
            "category": {
              "id": 8077,
    ...
    const deletePet = await request.delete(`${process.env.API_URL}pet/`+petId, {
        headers :{
            api_key : "special-key",
        }
    })
    ...
    const getPet = await request.get(`${process.env.API_URL}pet/`+petId);
